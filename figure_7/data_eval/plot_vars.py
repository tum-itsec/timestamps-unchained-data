...
def main():
	...
	x = np.array([(all_signals[i]['tx'] if all_signals[i]['source'] == 'a' else all_signals[i].get('rx', -1)) / 1_000_000_000_000 for i in range(0, max_id)])
	...
	ax.scatter(x, ftm_l_y, label=f"FTM-L (avg = {ftm_l_avg:.3f}m, std = {ftm_l_std:.3f}m)", color="blue", marker="o")
	ax.scatter(x, ftm_r_y, label=f"FTM-R (avg = {ftm_r_avg:.3f}m, std = {ftm_r_std:.3f}m)", color="green", marker="o")
	ax.scatter(x, dstwr_l_y, label=f"DSTWR-L (avg = {dstwr_l_avg:.3f}m, std = {dstwr_l_std:.3f}m)", color="red", marker="o")
	ax.scatter(x, dstwr_r_y, label=f"DSTWR-R (avg = {dstwr_r_avg:.3f}m, std = {dstwr_r_std:.3f}m)", color="orange", marker="o")


	ax.scatter(x, bad_ones, label=f"BAD ONES)", color="black", marker="x")

	# Add average lines
	ax.axhline(y=ftm_l_avg, color="blue", linestyle="--", alpha=0.5)
	ax.axhline(y=ftm_r_avg, color="green", linestyle="--", alpha=0.5)
	ax.axhline(y=dstwr_l_avg, color="red", linestyle="--", alpha=0.5)
	ax.axhline(y=dstwr_r_avg, color="orange", linestyle="--", alpha=0.5)
